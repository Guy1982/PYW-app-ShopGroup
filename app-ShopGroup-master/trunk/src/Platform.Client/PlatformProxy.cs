...
namespace Platform.Client
{
	public interface IPlatformProxy
	{
        ...
        T Get<T>(string servicePath, ICollection<KeyValuePair<string, object>> parameters);
	}
	...
	public class PlatformProxy : IPlatformProxy
	{
		...
		private ICollection<KeyValuePair<string, object>> GetParameters(object parametersModel)
		{
			...
			var modelPropertiesInfo = type.GetProperties(BindingFlags.Instance | BindingFlags.Public);
        ...
        public T Get<T>(string servicePath, ICollection<KeyValuePair<string, object>> parameters)
        {
            return Get<T>(GetServiceUrl(servicePath), parameters, AddContextParameters);
        }
